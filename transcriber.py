...
def carregar_modelo():
    ...
    if _modelo is None:
        ...
        # Check shared cache (Linux .deb installs model here)
        shared_cache = "/opt/ValidadorTelefones/.whisper_cache"
        download_root = shared_cache if os.path.isdir(shared_cache) else None
        _modelo = whisper.load_model(WHISPER_MODEL, download_root=download_root)
